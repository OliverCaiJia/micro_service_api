...
class UserSpreadNiwodaiBatchListener extends AppListener
{
    ...
    public function handle(UserSpreadBatchEvent $event)
    {
        try {
            ...
            SLogger::getStream()->error('你我贷延迟推送失败-catch');
    ...
    {
        if ($data['type_nid'] == SpreadNidConstant::SPREAD_NIWODAI_NID) {
            ...
            //推送数据
            $spreadParams = NiwodaiConfig::getParams($data);
            $res = NiwodaiService::apply($spreadParams);
            ...
            //更新spreadType
//            UserSpreadFactory::updateSpreadTypeIsSuccess($data['type_nid'], $params['status']);

            //更新spread
            UserSpreadFactory::updateSpreadByMobile($data['mobile']);

            //更新spreadLog
//            UserSpreadFactory::updateSpreadLogByTypeIdMobile($data, $params);
