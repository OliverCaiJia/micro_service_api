        ...
        try {
            ...
            if (!empty($type)) {
                ...
                if (!$spreadLogInfo || 2 == $spreadLogInfo['status']) {
                    ...
                    //SLogger::getStream()->info('你我贷', ['data' => $spread]);
                    $this->pushNiwodaiData($spread);
                }
            }
        } catch (\Exception $exception) {
            SLogger::getStream()->error('你我贷发送失败-catch');
...
//        }
    ...
    private function nowPush($spread = [])
    {
        ...
        // 更新推送次数等数据
        $spread['type_nid'] = SpreadNidConstant::SPREAD_NIWODAI_NID;
...
//        return false;
    ...
    {
        if ($data['salary'] != '001' && in_array($data['work_hours'], ['002', '003'])) {
