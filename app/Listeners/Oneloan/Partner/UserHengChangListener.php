...
class UserHengChangListener extends AppListener
{
        ...
        try {
            ...
            if (!empty($type)) {
                ...
                if (!$spreadLogInfo || 2 == $spreadLogInfo['status']) {
                    ...
                    //SLogger::getStream()->info('恒昌', ['data' => $spread]);
                    $this->pushHengChangData($spread);
                }
            }

        } catch (\Exception $exception) {
            SLogger::getStream()->error('恒昌发送失败-catch');
    ...
    private function nowPush($spread)
    {
        ...
        //SLogger::getStream()->info('hengchang', ['res' => $res]);
        if (isset($res['ResponseCode'])) {
            $params['message'] = HengchangConfig::getMessage($res['ResponseCode']);
    ...
    private function checkCondition($data)
    {
        ...
        {
            if (('001' == $data['occupation'] && in_array($data['work_hours'], ['002', '003']))) //2.工作类型
            {
                return true;
            }
        }
