        ...
        try {
            ...
            if (!empty($type)) {
                ...
                if (!$spreadLogInfo || 2 == $spreadLogInfo['status']) {
                    ...
                    //SLogger::getStream()->info('融时代', ['data' => $spread]);
                    $this->pushRongshidaiData($spread);
                }
            }
        } catch (\Exception $exception) {
            SLogger::getStream()->error('融时代发送失败-catch');
...
//        }
    ...
    {
        //推送融时代
        $res = RongshidaiService::spread($spread);
        ...
        // 更新推送次数等数据
        $spread['type_nid'] = SpreadNidConstant::SPREAD_RONGSHIDAI_NID;
